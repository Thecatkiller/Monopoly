...
//Acciones de auto completar 
$('#query').typeahead({
    prefetch: {
        url: 'GetExternalDestinationAccountToTypeAhead'
    }
});

$('.tt-query').css('background-color', '#fff');
$(".tt-dropdown-menu").prepend(
    $("<a>").attr({ "data-toggle": "modal", "data-target": ".bs-example-modal-lg", href: "javascript:;" }).addClass("transferirDestino").on("click", function () {
        $('#ExternalDestinationAccount').html('<div class="col-md-12 titulo_page"><center><img class="img-responsive" src="../Resources/img/2.gif"></center></div>');
        $('#ExternalDestinationAccount').load('ExternalDestinationAccount', function (response) { });
    }).append("Transferir a un nuevo destinatario")).on("click", ".tt-is-under-cursor", function () {

        var tmpText = $(this).children().text();
        var myarray = tmpText.split('|');
        var accountNumber = myarray[2];

        GetExternalDestinationAccountToTypeAheadSearchValues(accountNumber);

        $(".container_overlight3").fadeOut("fast");
        $('.modal-nuevo-destinatario').modal('hide');
        $(".BuscarContactos").hide();
        $(".listadosPagos-lista2").fadeIn();
        $(".select-chkT").prop("checked", true);
        $(".transferencias_inmediatas").fadeIn();
        $(".listadosPagos-lista").fadeOut();
        $(".seleccionPagos-lista").fadeIn();

    });

function GetExternalDestinationAccountToTypeAheadSearchValues(a) {
    var datos = [];
    datos = {
        accountNumber: a
    }
        
    var actionUrl = "GetExternalDestinationAccountToTypeAheadSearchValues";
    var data = JSON.stringify(datos);

    // Inicio Loading
    console.log('Loading');

    $('.PrestamosTerceros').prepend(
          $("<li id='Load'>").addClass("listadosPagos listadosPagos-lista2").attr("style", "display: table; padding: 0 24px 0 0;").append(
                $("<label>").attr({ "for": "destinoT", "data-toggle": "modal", "data-target": ".bs-example-modal-lg" }).addClass("pagoprestamos").append(
                $("<p style='margin-left:40px;'>").append(LoadingHTML))));
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            entityAccountDestino = { accountCCI: response.accountCCI, propia: false, propioBanco: false };

            desAccountAlias = response.alias;
            desAccountDescription = response.alias;
            desAccountNumber = response.accountNumber;
            desCCI = response.accountCCI;
            desBankName = response.bankDescription;
            desName = response.name + ' ' + response.surname;
            desDocumentType = response.documentType;
            desDocumentNumber = response.documentNumber;
            operationAccountBFP = 0;
            operationAccountOwn = 0;

            $("#Load").remove();

            $('.PrestamosTerceros').prepend(
            $("<li>").addClass("listadosPagos listadosPagos-lista2").attr("style", "display: table; padding: 0 24px 0 0;").append(
                $("<article>").addClass("listadosPagosRadios listadosPagos_Nuevo").append(
                    $("<input>").attr({ name: "destino", type: "radio", id: "destinoT" }).addClass("select-chkT").on("click", function () {
                        if (!$(this).hasClass('select-chk') && !$(this).hasClass('select-chkT')) {
                            $(".listadosPagos-lista").hide();
                            $(".seleccionPagos-lista").fadeIn();
                            //
                            $(".listadosPagos-lista2").hide();
                            $(".seleccionPagos-lista2").fadeIn();
                            ...
                            //
                            $(".boton_NuevaCuentas-propio").removeAttr("data-dismiss");
                            $(".boton_NuevaCuentas-OtroBanco").removeAttr("data-dismiss");
                            $(".boton_accione-financiero").removeAttr("data-dismiss");
                        }
                        $(this).prop("checked", true);
                    }),

                    $("<label>").attr({ "for": "destinoT", "data-toggle": "modal", "data-target": ".bs-example-modal-lg" }).addClass("pagoprestamos").append(
                        $("<p>").append(desAccountAlias),
                        $("<span>").append("Nro " + desAccountNumber)),
                    $("<em>").addClass("cerrarCuentaPagarNueva cerrarTercero").append("&times;").on("click", function () {
                        $(".listadosPagos-lista2").fadeOut();
                        $(".seleccionPagos-lista2").fadeIn();
                        $(".BuscarContactos").fadeIn();
                        $(".select-chkT").prop("checked", false);
                        $('.PrestamosTerceros li').remove(".listadosPagos-lista2");
                        $('#query').val('');
                        $('.continuar__boton').prop('disabled', true);
                        $('.continuar__boton').removeClass('siguiente__pasos--active');
                    })
            )));
...
//Fin de acciones de auto completar 